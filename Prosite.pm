...
# Authors: 
#   Alexandre Gattiker
#   Elisabeth Gasteiger; E-mail: Elisabeth.Gasteiger@sib.swiss
...
use strict;
@ISA = qw(Exporter);
@EXPORT = qw(scanPattern scanProfiles parseXPSA prositeToRegexp_old 
    prositeToRegexp checkPatternSyntax parseProsite);
$VERSION = 1.0;
...
=item scanProfiles $filename, $level_min, 
...
matched cut-off level, respectively. The return value is a reference to an hash for 
which the keys are PROSITE accession numbers and the values are arrays of arrays; for
...
Elisabeth Gasteiger, Elisabeth.Gasteiger@sib.swiss

Edouard de Castro, ecastro@sib.swiss
